...
class AddAttendee
{
	...
	function AddAttendee($secretAcessKey,$access_key,$webServiceUrl,$class_id,$XMLAttendee,$id)
	{
		...
		$requestParameters["class_id"] = $class_id;//"2740286";//required 
		$requestParameters["attendee_list"]=(string)$XMLAttendee;
				...
				{    
					...
					//echo "<br>attendee_url=".$attendee_url=$attendee_urlTag->item($i)->nodeValue;
                                DB::table("appointments")->where(['id'=>$id])->update([
                                        'customer_meeting_url'=>$attendee_urlTag->item($i)->nodeValue, 
                                        ]); 
				}
 			}
